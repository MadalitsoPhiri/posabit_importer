...
export const handleCustomerLoyaltySync = async (
  customerData: GreenWellnessCustomer
) => {
...
export const handleDiffSync = async (
  customerData: GreenWellnessCustomer,
  ...
  try {
    ...
    if (customerData.PointBalance > DigitalWalletCard.balance.bonusBalance) {
      ...
      await addToProgram(
        ...
        customerData.PointBalance - DigitalWalletCard.balance.bonusBalance
      );
    } else if (customerData.PointBalance < DigitalWalletCard.balance.balance) {
      console.log("subtracting from program");

      await subtractFromProgram(
        DigitalWalletCard.id,
        DigitalWalletCard.balance.bonusBalance - customerData.PointBalance
      );
    }
  } catch (e) {
    console.log("error", e);
